...
func WriteTemplatesToFiles(results []templateResult, outputDir string) (*TemplateWriteStats, error) {
	...
	for _, tmpl := range results {
		...
		for _, variant := range tmpl.Variants {
			...
			variantJSON, err := json.MarshalIndent(variant, "", "  ")
			if err != nil {
				debugErrorLog("Error: %s", err)
				stats.Errors = append(stats.Errors, fmt.Sprintf("Failed to marshal variant '%s/%s' for '%s': %v", channel, variantName, tmpl.Slug, err))
				continue
			}
