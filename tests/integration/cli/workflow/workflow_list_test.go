...
func TestWorkflowList_Success(t *testing.T) {
	listData := readTestdata(t, "workflows", "workflow_list_response.json")
	...
	}, "workflow", "list", "--workspace", "test-ws")
	...
	for _, slug := range []string{"workflow-one", "workflow-two"} {
...
func TestWorkflowList_DefaultLimitOffsetMode(t *testing.T) {
	listData := readTestdata(t, "workflows", "workflow_list_response.json")
...
func TestWorkflowList_CustomLimitOffset(t *testing.T) {
	listData := readTestdata(t, "workflows", "workflow_list_response.json")
	...
	_, _, exitCode := runCLIWithEnv(t, map[string]string{
		"SUPRSEND_SERVICE_TOKEN": "test-token",
		"SUPRSEND_MGMNT_URL":     server.URL,
	}, "workflow", "list", "--workspace", "test-ws", "--limit", "5", "--offset", "10")
...
func TestWorkflowList_DraftMode(t *testing.T) {
	listData := readTestdata(t, "workflows", "workflow_list_response.json")
	...
	_, _, exitCode := runCLIWithEnv(t, map[string]string{
		"SUPRSEND_SERVICE_TOKEN": "test-token",
		"SUPRSEND_MGMNT_URL":     server.URL,
	}, "workflow", "list", "--workspace", "test-ws", "--mode", "draft")
...
func TestWorkflowList_APIError(t *testing.T) {
	server := newMockServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
